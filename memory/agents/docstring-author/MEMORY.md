...
- [vrcpilot.speaker.linux per-PID design docstring patterns](project_speaker_linux_per_pid.md) — Phase A null-sink-only routing (no loopback), diagnostic contract (candidate / post-verify / pw-record stderr), teardown order, trailing-`\s` anchor, ExitStack callback-name preservation
