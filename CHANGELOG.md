...
- **`vrcpilot.speaker.routing` subpackage and `vrcpilot speaker` CLI**: cross-platform user-space relay that forwards a single VRChat PID's output audio to an arbitrary output device. The public surface is `AudioDevice` / `list_devices` / `default_device` / `find_device` / `Router` / `route` / `AudioRoutingError` / `DeviceNotFoundError` (8 symbols). Windows reuses the existing `proc-tap` per-PID capture and Linux reuses the native PipeWire pipeline; both then push the captured stream through `soundcard` into the chosen output device. The OS-level app→device mapping (Windows `IAudioPolicyConfig`, EarTrumpet) is never touched, so per-PID isolation holds even when several `VRChat.exe` processes are running simultaneously. The CLI exposes two actions: `vrcpilot speaker list` (enumerate output devices as YAML) and `vrcpilot speaker route --pid N [--device <id|name>] [--chunk-seconds 0.02] [--blocksize N]` (foreground relay, Ctrl+C to stop). Omitting `--device` routes to the OS default speaker. See [`docs/virtual-audio.md`](docs/virtual-audio.md) for setup recipes covering virtual speaker devices (VB-Cable / Voicemeeter / VAC / PipeWire null-sinks) and complementary GUI tools (Windows: EarTrumpet; Linux: pavucontrol / Helvum / qpwgraph).
- **`vrcpilot linux-mic` gains `--suffix <NAME>` / `--all`**, primarily so multiple AI-agent instances can run in parallel with each instance owning a dedicated virtual mic instead of contending for a single shared `VRCPilotMic`. Suffixed sinks (e.g. `VRCPilotMic_bot`) can now be registered side by side; omitting `--suffix` preserves the original `VRCPilotMic` sink name. `status --all` / `unregister --all` enumerate / bulk-remove every `vrcpilot-mic*.conf` under the PipeWire config directory.

### Fixed

- **Linux speaker now isolates audio per PID, fixing the regression where recording several VRChat instances in parallel produced all-silent WAVs.** The capture path was rebuilt around two stages of explicit global-port-id links (`pw-link`): (1) the VRChat stream's output ports → the per-PID null-sink (`vrcpilot_tap_<pid>`) playback ports, and (2) the tap's monitor ports → the input ports of a `pw-record` spawned with `--target=0`. The root cause was twofold and both were Wireplumber session policies: the old `sink_input_move` (i.e. `target.object` metadata manipulation) was reverted after the call reported success, and the name-based `pw-record --target=<sink>.monitor` was redirected to the default sink's monitor whenever more than one sink existed. Explicit global-port-id links are not subject to Wireplumber's policy and stay unique even when two instances share the same node name (`VRChat.exe`), so isolation is reliable. As a side effect the VRChat stream stays linked to the default sink as well, so **the user keeps hearing VRChat audio while recording is in progress**.
- **`vrcpilot speaker list` / `ocr` / `detect` no longer escape non-ASCII characters in their YAML output.** The previous behaviour emitted `\uXXXX` sequences (because `yaml.safe_dump` defaults to `allow_unicode=False`) and on Windows could additionally be transcoded by the terminal codepage (e.g. cp932) into mojibake. Output now goes through a shared `emit_yaml(payload)` helper that writes raw UTF-8 bytes directly to `sys.stdout.buffer`, bypassing any codepage conversion, so piped / file-redirected output is valid UTF-8 YAML that round-trips through `yaml.safe_load` on any locale.
...
- **`process.py` is now a package.** `vrcpilot/process.py` was split into `vrcpilot/process/__init__.py`, `vrcpilot/process/launch.py`, `vrcpilot/process/executable.py` (cross-platform launcher discovery), and `vrcpilot/process/linux.py` (Linux-only `find_umu_launcher` / `profile_wineprefix` / `preflight_linux_environment`; raises `ImportError` on Windows). `from vrcpilot.process import ...` keeps working through the package's re-exports, but any code that imported `vrcpilot.process` expecting a single-file module attribute (e.g. `vrcpilot.process.__file__` pointing at a `.py` file) needs updating.
- **Top-level `vrcpilot.*` namespace narrowed to user-facing API.** The following symbols are no longer re-exported from `vrcpilot` and must be imported from their subpackage instead: `Capture` (use `vrcpilot.capture.Capture`), `Speaker` / `SpeakerLoop` / `AudioCallback` (`vrcpilot.speaker.*`), `OCREngine` / `OCRResult` / `OCRWord` / `RapidOCREngine` (`vrcpilot.ocr.*`), `DetectEngine` / `DetectResult` / `Detection` / `TemplateDetectEngine` (`vrcpilot.detect.*`), `ensure_target` / `InputController` (`vrcpilot.controls.*` / `vrcpilot.osc.*`), `AvatarParameters` (`vrcpilot.osc.AvatarParameters`), `resolve_pid` (`vrcpilot.process.resolve_pid`), and the narrow-scope exceptions `UmuLauncherNotFoundError` / `VRChatLauncherNotFoundError` / `VRChatAlreadyRunningError` / `VRChatDisplayNotAvailableError` / `SteamNotRunningError` (all under `vrcpilot.process.*` / `vrcpilot.steam.*`). The retained top-level surface is the everyday user-facing API: `launch`, `terminate`, `find_pid`, `find_pids`, `OscConfig`, `VRChatMultipleInstancesError`, `VRChatNotRunningError`, `VRChatNotFocusedError`, `SteamNotFoundError`, `MicDeviceNotFoundError`, `CaptureLoop`, `Mic`, `Screenshot`, `take_screenshot`, `focus`, `is_foreground`, `unfocus`, `ocr`, `detect`, `Key`, `MouseButton`, `keyboard`, `mouse`, `clipboard`, `OscSender`.
- **`vrcpilot.controls.{mouse,keyboard}.__all__` no longer re-export `ensure_target`.** The canonical path is `vrcpilot.controls.ensure_target` (still exported). `from vrcpilot.controls.mouse import ensure_target` and `from vrcpilot.controls.keyboard import ensure_target` keep working (PEP 484 explicit re-export), but star-imports of these subpackages no longer pull in `ensure_target`.
...
- **New exceptions in `vrcpilot.process`**: `VRChatMultipleInstancesError` (subclass of `VRChatNotRunningError`, carries `pids: list[int]`; re-exported at top level alongside `VRChatNotRunningError`), `VRChatLauncherNotFoundError`, `VRChatAlreadyRunningError`, `UmuLauncherNotFoundError`. The launcher / already-running / umu-not-found exceptions stay scoped to `vrcpilot.process.*` (see the namespace-narrowing entry above). `UmuLauncherNotFoundError.args[0]` includes installation hints for Ubuntu (deb from the [umu-launcher releases page](https://github.com/Open-Wine-Components/umu-launcher/releases/latest)) and a fallback suggestion to use `--via-steam`.
- **Launcher auto-discovery**: `vrcpilot.process.find_vrchat_launcher(override=None)` locates the EAC wrapper `launch.exe` via (1) `override` / `--vrchat-launcher`, (2) `$VRCHAT_LAUNCHER`, (3) Steam `libraryfolders.vdf` lookup (registry on Windows, `~/.steam/steam/steamapps/libraryfolders.vdf` / `~/.local/share/Steam/...` on Linux), (4) standard install paths. The Linux-only `umu-run` resolver lives as an internal helper at `vrcpilot.process.linux.find_umu_launcher(override=None)` (no stability guarantee; `vrcpilot.process.linux` raises `ImportError` on Windows).
...
- **`vrcpilot.controls.mouse.Mouse.click(interval=...)`** new keyword-only argument: per-cycle inter-press gap in seconds, applied between cycles only when `count > 1` and `interval > 0` (`count <= 1` or non-positive `interval` is a complete no-op; the final cycle has no trailing sleep). Default `0.0` preserves the previous back-to-back behaviour. Required on Linux + Wine + VRChat for `count=2` to be recognised as a double-click — Wine silently coalesces inter-cycle-zero repeats into a single click; empirically 0.02-0.05s is the working range. The module-level wrapper `vrcpilot.controls.mouse.click` and CLI `vrcpilot mouse click --interval FLOAT` mirror the new argument.
...
- **Linux pre-flight check for direct-spawn `launch()`**. The direct-spawn path now refuses to spawn when neither `$DISPLAY` nor `$WAYLAND_DISPLAY` is set, raising the new `VRChatDisplayNotAvailableError` (CLI exit 2). Otherwise `umu-run` would hang waiting for a display server (the symptom seen when invoking `vrcpilot launch` over plain SSH). Additionally, the direct-spawn path no longer forwards `GAMEID=<app_id>` to the `umu-run` subprocess: ProtonFixes applied its global defaults (because VRChat has no per-title fix registered), which on UMU-Proton-latest crashed wine via unimplemented `coremessaging.dll.DllGetActivationFactory`. Without `GAMEID`, `umu-run` uses its own neutral default prefix and starts cleanly. Linux-only; Windows is unchanged.
- **Linux pre-flight check for `launch(via_steam=True)`**. The `via_steam=True` path now checks that the Steam desktop client process is actually running on the host and raises the new `SteamNotRunningError` (CLI exit 2) when it isn't — `steam.exe -applaunch` cannot start the Steam UI itself. Linux-only; Windows is unchanged. New helper `vrcpilot.steam.is_steam_running()` is public (accessible as `vrcpilot.steam.is_steam_running`).

### Changed

- `vrcpilot/process.py` split into a package: `vrcpilot/process/__init__.py` (`find_pids`, `resolve_pid`, `terminate`, `wait_for_pid`, `wait_for_no_pid`, exception hierarchy, constants), `vrcpilot/process/launch.py` (`launch`, `OscConfig`, `build_*`, `validate_launch_args`), `vrcpilot/process/executable.py` (`find_vrchat_launcher`; cross-platform), and `vrcpilot/process/linux.py` (Linux-only `find_umu_launcher` / `profile_wineprefix` / `preflight_linux_environment`). All public symbols remain importable from `vrcpilot.process`.
...
- **First synthetic keyboard / mouse event no longer silently dropped on Linux.** `vrcpilot.controls.keyboard.linux.LinuxKeyboard.__init__` and `vrcpilot.controls.mouse.linux.LinuxMouse.__init__` now sleep 0.5s after `inputtino.{Keyboard,Mouse}()` constructs the `/dev/uinput` device, giving X11 / libinput time to subscribe to the new device through udev. Without this grace period the first event was emitted into evdev before any listener had bound, and the kernel silently dropped it — visible only under "warmup → first synthetic input" patterns (`tests/e2e/{osc,detect}.py`); CLI invocations happened to absorb the latency in Python startup overhead and never tripped on it. Linux-only; Windows uses `SendInput` and has no equivalent hot-plug latency.
