...
- **キャプチャ系**: `capture/`（`Capture` + `CaptureLoop` + `Mp4FrameSink` / `Y4mStdoutFrameSink`、Windows / Linux バックエンド）、`screenshot`（GUI 自動化向けの 1 ショット取得。`Screenshot.save/load` は file-path / inline base64 の 2 モード対応で YAML を双方向にやり取り可能）
...
- **音声系**: `speaker/`（`Speaker` + `SpeakerLoop`、Linux は PipeWire ネイティブバックエンド (`speaker/linux.py`)、Windows は `proc-tap` 経由のプロセス単位ループバックバックエンド (`speaker/windows.py`)。`speaker/session.py` の `_select_speaker_backend` で 2 択にディスパッチ、Windows / Linux 以外は `NotImplementedError`）
