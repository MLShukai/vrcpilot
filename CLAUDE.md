...
- **CLI フロントエンド**: `cli/` 配下にサブコマンド毎 1 ファイル（`launch` / `pid` / `terminate` / `focus` / `unfocus` / `screenshot` / `record` / `mic` / `speaker-device` / `mouse` / `keyboard` / `paste` / `ocr` / `detect` / `osc`）。`speaker-device` は `list` / `route` の 2 アクションを持ち、PID 単位の音声リレーを foreground で実行する。ディスパッチは `cli/__init__.py` の `build_parser` / `main`、共有ヘルパは `cli/_common.py`（`add_screenshot_input_arg` / `resolve_screenshot` で `--screenshot` ↔ stdin pipe の入力解決を集約）。Linux 限定の `linux-mic` サブコマンドは `cli/__init__.py` 内で `if sys.platform == "linux":` ブロックで条件付き登録される（Windows では `vrcpilot --help` にも現れない）
