...
``vrcpilot.controls.mouse.get_vrchat_window_rect`` to a fixed rectangle
so tests that exercise the window-local -> desktop translation in
:meth:`Mouse.move` get a deterministic origin without needing a real
VRChat process. Coordinates assume the window occupies
