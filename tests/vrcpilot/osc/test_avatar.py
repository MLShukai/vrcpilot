...
class TestPrefix:
    def test_avatar_parameter_prefix_value(self):
...
class TestSendBool:
    ...
    def test_forwards_to_sender_send_bool(
        self, avatars: AvatarParameters, sender_mock, name: str, value: bool
    ):
    ...
    def test_passes_value_through_unchanged(
        self, avatars: AvatarParameters, sender_mock
    ):
        """The view should not normalize the bool itself; OscSender does
        ``int(bool(...))`` once, centrally."""
        avatars.send_bool("Foo", True)
        sender_mock.send_bool.assert_called_once_with(
            AVATAR_PARAMETER_PREFIX + "Foo", True
        )

    @pytest.mark.parametrize("bad_name", ["", "foo/bar", "/foo", "foo/", "a/b/c"])
    def test_rejects_invalid_name(
        self, avatars: AvatarParameters, sender_mock, bad_name: str
    ):
...
class TestSendInt:
    ...
    def test_forwards_to_sender_send_int(
        self, avatars: AvatarParameters, sender_mock, name: str, value: int
    ):
        avatars.send_int(name, value)
        sender_mock.send_int.assert_called_once_with(
            AVATAR_PARAMETER_PREFIX + name, value
        )
        sender_mock.send_bool.assert_not_called()
        sender_mock.send_float.assert_not_called()

    def test_propagates_sender_range_error(
        self, avatars: AvatarParameters, sender_mock
    ):
...
class TestSendFloat:
    ...
    def test_forwards_to_sender_send_float(
        self, avatars: AvatarParameters, sender_mock, name: str, value: float
    ):
...
class TestValidNames:
    ...
    def test_valid_names_pass_through(
        self, avatars: AvatarParameters, sender_mock, good_name: str
    ):
