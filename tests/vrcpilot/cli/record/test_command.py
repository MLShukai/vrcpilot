"""Integration-with-fakes tests for :mod:`vrcpilot.cli.record`.

The record CLI composes three project-owned abstractions:

* :class:`vrcpilot.cli.record.muxer.BaseMuxer` (file or stdout muxer)
* :class:`vrcpilot.capture.CaptureLoop` (background frame producer)
* :class:`vrcpilot.speaker.SpeakerLoop` (background audio chunk
  producer)

All three are project-defined ABCs, so the fakes in
:mod:`tests.fakes` (``FakeMp4FileMuxer`` / ``FakeWavFileMuxer`` /
``FakeMkvStdoutMuxer`` / ``FakeCaptureLoop`` / ``FakeSpeakerLoop``)
are legitimate test seams under the new mocking policy. Each fake
records constructor kwargs, ``write_*`` calls, and ``closed`` state
so the CLI's wiring contract can be asserted end-to-end without
invoking PyAV / Win32 WGC / proc-tap.

Per the policy, ``time.sleep`` is NOT patched. The CLI's duration
gate uses real ``time.sleep`` with ``--duration 0.0`` for an
immediate return; tests requiring the KeyboardInterrupt branch
(``--duration`` omitted) cannot be covered without patching sleep
and are deferred to the e2e suite.

``sys.stdout.isatty()`` IS patched in the pipe-mode tests; while
``sys.stdout`` is a stdlib object, the CLI inspects it through
``sys.stdout.isatty()`` to decide whether stdout is safe for binary
MKV output, and pytest's stdout replacement makes the unpatched
...
def record_fakes(mocker: MockerFixture) -> _RecordFakes:
    """Wire the project's record-side ABCs to their canonical fakes.

    Resets class-level state on every fake so ``instances`` lists do
    not leak between tests. Also pins ``sys.stdout.isatty()`` to
    ``False`` so the no-``-o`` path enters pipe mode regardless of how
    the test runner captures stdout (pytest's capfd replaces stdout
    with a non-TTY buffer; the production check still needs to see
    the False signal).
    """
...
def _argv(*extra: str) -> list[str]:
    """Build a record argv that finishes immediately via ``--duration 0``.

    Real ``time.sleep(0.0)`` returns instantly without any patching;
    the fakes' ``start()`` methods synchronously emit their frames so
    the muxer has data before the duration sleep returns.
    """
    return ["record", *extra, "--duration", "0"]
...
class TestModeDispatch:
    """Cover the (--video, --audio) x (stdout, dir, file) truth table.

    Asserts (a) which fake muxer was constructed, (b) the
    ``video`` / ``audio`` constructor flags on that muxer, and (c)
    which of the two loops were started so the ExitStack driving
    logic stays honest.
    """
...
class TestExtensionValidation:
    ...
    def test_video_mode_rejects_wav(
        ...
        assert main(_argv("--video", "-o", str(out))) == 2
        captured = capsys.readouterr()
    ...
    def test_audio_mode_rejects_mp4(
        ...
        assert main(_argv("--audio", "-o", str(out))) == 2
        captured = capsys.readouterr()
    ...
    def test_directory_picks_default_filename_for_mode_both(
        ...
        assert main(_argv("-o", str(tmp_path))) == 0
        assert len(record_fakes.mp4.instances) == 1
        ...
        assert capsys.readouterr().out.strip() == str(sink.output_path.resolve())
    ...
    def test_directory_picks_default_filename_for_mode_video(
        ...
        assert main(_argv("--video", "-o", str(tmp_path))) == 0
        assert len(record_fakes.mp4.instances) == 1
    ...
    def test_directory_picks_default_filename_for_mode_audio(
        ...
        assert main(_argv("--audio", "-o", str(tmp_path))) == 0
        assert len(record_fakes.wav.instances) == 1
...
class TestStdoutMode:
    def test_no_output_streams_mkv_for_both(
        self,
        record_fakes: _RecordFakes,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
...
class TestFpsHandling:
    ...
    def test_default_fps_is_30_in_file_mode(
        ...
        assert main(_argv("-o", str(tmp_path / "x.mp4"))) == 0
    ...
    def test_explicit_fps_propagates_to_mp4_muxer_and_loop(
        ...
        assert main(_argv("--fps", "60", "-o", str(tmp_path / "x.mp4"))) == 0
    ...
    def test_explicit_fps_with_video_only(
        ...
        assert main(_argv("--video", "--fps", "60", "-o", str(tmp_path / "x.mp4"))) == 0
        assert record_fakes.mp4.instances[0].fps == 60.0
        assert record_fakes.capture_loop.instances[0].fps == 60.0

    def test_explicit_fps_with_stdout_mkv(self, record_fakes: _RecordFakes) -> None:
        assert main(_argv("--fps", "60")) == 0
    ...
    def test_fps_with_audio_only_rejected(
        ...
        assert main(_argv("--audio", "--fps", "60", "-o", str(tmp_path / "x.wav"))) == 2
        captured = capsys.readouterr()
        assert (
...
class TestErrors:
    def test_capture_loop_runtime_error_returns_one(
        ...
        assert main(_argv("-o", str(tmp_path / "err.mp4"))) == 1
        captured = capsys.readouterr()
    ...
    def test_speaker_loop_runtime_error_returns_one(
        ...
        assert main(_argv("--audio", "-o", str(tmp_path / "err.wav"))) == 1
        captured = capsys.readouterr()
        assert "vrcpilot: VRChat is not running" in captured.err
        assert captured.out == ""
    ...
    def test_zero_frames_and_samples_returns_one(
        ...
        assert main(_argv("-o", str(tmp_path / "zero.mp4"))) == 1
        captured = capsys.readouterr()
    ...
    def test_video_only_with_zero_frames_returns_one(
        ...
        assert main(_argv("--video", "-o", str(tmp_path / "z.mp4"))) == 1
        assert "no frames or audio samples were recorded" in capsys.readouterr().err
    ...
    def test_audio_only_with_zero_samples_returns_one(
        ...
        assert main(_argv("--audio", "-o", str(tmp_path / "z.wav"))) == 1
        assert "no frames or audio samples were recorded" in capsys.readouterr().err


# ---------------------------------------------------------------------------
# Callback wiring + muxer closure
# ---------------------------------------------------------------------------
...
class TestCallbackWiring:
    def test_video_callback_writes_to_muxer(
        ...
        assert main(_argv("-o", str(tmp_path / "cb.mp4"))) == 0
        muxer = record_fakes.mp4.instances[0]
    ...
    def test_audio_callback_writes_to_muxer(
        ...
        assert main(_argv("--audio", "-o", str(tmp_path / "cb.wav"))) == 0
    ...
    def test_both_loops_feed_one_muxer(
        ...
        assert main(_argv("-o", str(tmp_path / "both.mp4"))) == 0
        muxer = record_fakes.mp4.instances[0]
    ...
    def test_muxer_is_closed_after_run(
        ...
        assert main(_argv("-o", str(tmp_path / "c.mp4"))) == 0
...
class TestProgressMessaging:
    def test_progress_lands_on_stderr_not_stdout(
        self,
        record_fakes: _RecordFakes,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ...
    def test_pipe_mode_emits_no_path_line(
        self,
        record_fakes: _RecordFakes,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
...
class TestPidArg:
    ...
    def test_passes_pid_to_both_loops(
        ...
        assert main(_argv("-o", str(tmp_path / "pid.mp4"), "--pid", "8765")) == 0
    ...
    def test_no_pid_passes_none(
        ...
        assert main(_argv("-o", str(tmp_path / "nopid.mp4"))) == 0
    ...
    def test_video_only_passes_pid_to_capture(
        ...
        assert (
            main(_argv("--video", "-o", str(tmp_path / "v.mp4"), "--pid", "111")) == 0
        )
    ...
    def test_audio_only_passes_pid_to_speaker(
        ...
        assert (
            main(_argv("--audio", "-o", str(tmp_path / "a.wav"), "--pid", "222")) == 0
        )
        assert record_fakes.speaker_loop.instances[0].pid == 222
        assert record_fakes.capture_loop.instances == []

    def test_multi_instance_exits_with_diagnostic(
        self,
        record_fakes: _RecordFakes,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
        ...
        assert main(_argv("-o", str(tmp_path / "multi.mp4"))) == 1
        captured = capsys.readouterr()
        assert (
