...
| `launch_terminate`     | API (`vrcpilot.launch` / `find_pids` / `terminate`) のハッピーパス                                                                                                                                                                                                                                                                                                                                     |
...
| `multi_instance`       | `profile=1` と `profile=2` で 2 インスタンスを並走させ、複数 PID 系 API 契約 (`find_pids` newest-first / `VRChatMultipleInstancesError` / `VRChatAlreadyRunningError` / 個別 `terminate(pid)`) を検証する。Linux は profile ごとに wineprefix が auto-gen されるため初回は Proton 初期化で数分かかる (`profile=0` は Linux では Steam 管理 compatdata の共有スロットなので isolation 検証には使わない) |
...
| 名前                      | 内容                                                                                                                                                                                                                                                                                     |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cli_record_av`           | `vrcpilot record -o <mp4>` の既定 (映像 + 音声) ファイル出力経路。外部 encoder を介さず直接 mp4 に書き出す。`ffprobe` のみ使用（ffmpeg pipe なし）で h264 video stream 1 本 + aac audio stream 1 本と duration を確認                                                                    |
...
| `cli_record_ffmpeg`       | `vrcpilot record` の既定 (映像 + 音声) stdout MKV を `ffmpeg -i - -c:v copy -c:a copy` にパイプして `.mp4` に remux。`ffprobe` で h264 video 1 本 + aac audio 1 本 + duration を確認。file-mode 版は `cli_record_av` がカバー                                                            |
